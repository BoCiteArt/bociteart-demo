...
(function(){
  ...
  const MAX_LOCAL_OPERATIONS = 250;
  ...
  function sportFinanceId(prefix){
    const head = sportFinanceText(prefix) || "sport-finance";
  ...
  function sportFinanceDigits(value){
    return sportFinanceText(value).replace(/\D/g, "");
  }

  function sportFinanceEmailLooksValid(value){
    const email = sportFinanceText(value);
  ...
  /* =========================================================
     SPORT — CLUB / SESSION / ASSOCIATIONS
     ========================================================= */
  ...
  function sportFinanceSession(){
    ...
    if(
      window.BociteSportModule &&
      typeof window.BociteSportModule.getSession === "function"
    ){
      source = window.BociteSportModule.getSession() || {};
    }else{
      source = window.bociteartSportSession || {};
    }
    ...
    return {
      ...
      name:
        sportFinanceText(source.name),

      role:
        sportFinanceText(source.role),

      team:
        sportFinanceText(source.team)
    };
  }
  ...
  function sportFinanceClubSnapshot(club){
    ...
    return {
      clubRef:
        sportFinanceText(source.clubRef),
      ...
      officialName:
        sportFinanceText(source.officialName),

      commune:
        sportFinanceText(source.commune),

      organizationType:
        sportFinanceText(source.organizationType),

      legalForm:
        sportFinanceText(source.legalForm),
      ...
      rnaNumber:
        sportFinanceText(source.rnaNumber),

      vatStatus:
        sportFinanceText(source.vatStatus),

      vatNumber:
        sportFinanceText(source.vatNumber),

      accountingEmail:
        sportFinanceText(source.accountingEmail)
    };
  }
  ...
  function sportFinanceAssociationSnapshot(association){
    ...
    return {
      id:
        sportFinanceText(association.id),
      ...
      legalName:
        sportFinanceText(association.legalName),

      accountingEmail:
        sportFinanceText(association.accountingEmail),
  ...
  function sportFinanceInitialProfile(){
    const shared =
      window.BOCITEART_LAST_SPORT_MERCHANT_PROFILE;
  ...
  function sportFinanceValue(id){
    const field = sportFinanceField(id);
  ...
  function sportFinanceReadProfile(){
    return {
      name:
        sportFinanceValue("bcfSportMerchantName"),

      sirenSiret:
        sportFinanceValue("bcfSportMerchantSiret"),

      address:
        sportFinanceValue("bcfSportMerchantAddress"),

      phone:
        sportFinanceValue("bcfSportMerchantPhone"),

      email:
        sportFinanceValue("bcfSportMerchantEmail"),

      accountingEmail:
        sportFinanceValue("bcfSportMerchantAccountingEmail"),

      website:
        sportFinanceValue("bcfSportMerchantWebsite"),
  ...
  function sportFinanceMerchantSnapshot(profile){
    ...
    return {
      ...
      sirenSiret:
        sportFinanceText(source.sirenSiret),

      address:
        sportFinanceText(source.address),

      phone:
        sportFinanceText(source.phone),

      email:
        sportFinanceText(source.email),

      accountingEmail:
        sportFinanceText(source.accountingEmail),

      website:
        sportFinanceText(source.website),

      identityVersion:
        sportFinanceText(source.updatedAt)
    };
  }

  /* =========================================================
     OPÉRATIONS LOCALES DE SUIVI

     Ce journal facilite la préproduction et l'affichage.
     En production, la référence comptable officielle reste serveur.
     ========================================================= */

  function sportFinanceReadOperations(){
    try{
      const parsed = JSON.parse(
        window.localStorage.getItem(OPERATIONS_KEY) || "[]"
      );
  ...
  function sportFinanceWriteOperations(operations){
    const list = Array.isArray(operations)
      ? operations.slice(-MAX_LOCAL_OPERATIONS)
      : [];
  ...
  function sportFinanceUpsertOperation(data){
    ...
    const draftId =
      sportFinanceText(source.draftId);

    const operationRef =
      sportFinanceText(source.operationRef);

    const paymentReference =
      sportFinanceText(source.paymentReference);
    ...
    const index =
      operations.findIndex(function(item){
        return !!(
          (
            draftId &&
            sportFinanceText(item.draftId) === draftId
          ) ||
          (
            operationRef &&
            sportFinanceText(item.operationRef) === operationRef
          ) ||
          (
            paymentReference &&
            sportFinanceText(item.paymentReference) === paymentReference
          )
        );
      });
    ...
    if(!next.createdAt){
      next.createdAt = sportFinanceNow();
    }
    ...
    sportFinanceWriteOperations(operations);
  ...
  function sportFinanceStatusLabel(status){
    ...
    if(
      core &&
      typeof core.statusLabel === "function"
    ){
    ...
    return labels[sportFinanceText(status)] ||
      "État en cours";
  }

  function sportFinanceRenderLatestOperation(){
    const target =
      sportFinanceField("bcfSportLastOperation");
    ...
    const amount =
      Number(operation.amountHT || 0);

    target.innerHTML = `
      <strong>${sportFinanceEscape(
        sportFinanceStatusLabel(operation.status)
      )}</strong>
      ...
      ${
        Number.isFinite(amount) && amount > 0
          ? " — " +
            sportFinanceEscape(
              sportFinanceFormatMoney(amount)
            ) +
            " € HT"
          : ""
      }
      ...
      ${
        operation.paymentReference
          ? "<br>Réf. paiement : " +
            sportFinanceEscape(operation.paymentReference)
          : ""
      }
      ...
      ${
        operation.operationRef
          ? "<br>Réf. dossier : " +
            sportFinanceEscape(operation.operationRef)
          : ""
      }
    `;
  }
  ...
  /* =========================================================
     MONTANT / DESTINATION / APERÇU
     ========================================================= */

  function sportFinanceAmount(){
    const amount =
      Number(
        sportFinanceValue("bcfSportAmountHT")
      );
  ...
  function sportFinanceSelectedAssociation(){
    const id =
      sportFinanceValue("bcfSportAssociation");
  ...
  function sportFinanceSetStatus(message, state){
    const target =
      sportFinanceField("bcfSportStatus");
  ...
  function sportFinanceToggleAssociation(){
    ...
    const box =
      sportFinanceField("bcfSportAssociationBox");
    ...
    if(choice !== "HALF_HALF"){
      const select =
        sportFinanceField("bcfSportAssociation");
  ...
  /* =========================================================
     ...
     Le serveur devra répéter toutes les validations utiles.
     ========================================================= */
  ...
  function sportFinanceValidate(
    ...
    const digits =
      sportFinanceDigits(profile.sirenSiret);
    ...
    if(!profile.accountingEmail){
      ...
      !sportFinanceEmailLooksValid(profile.accountingEmail)
    ){
      errors.push(
    ...
    if(
      profile.email &&
      !sportFinanceEmailLooksValid(profile.email)
    ){
      errors.push(
  ...
  /* =========================================================
     POLITIQUES TRANSMISES AU SERVEUR

     Elles indiquent ce qui doit être contrôlé.
     Elles ne contiennent aucune formule financière secrète.
     ========================================================= */

  function sportFinancePolicies(){
    return {
      payment:{
        activationOnlyAfterConfirmedPayment:true,
        finalConfirmationSource:"server_psp",
        clientConfirmationNotSufficient:true,
        idempotencyRequired:true
      },

      publication:{
        durationDays:PUBLICATION_DAYS,
        startOnlyAfterConfirmedPayment:true
      },

      accounting:{
        finalProcessing:"server_only",
        reconciliationRequired:true,
        providerFeesRecorded:true,
        beneficiarySettlementServerSide:true,
        noPublicInternalFormula:true
      },

      documents:{
        merchantDocumentRequired:true,
        clubDocumentAccordingToLegalStatus:true,
        associationDocumentAccordingToEligibility:true,
        bociteartDocumentAccordingToActualService:true,
        duplicateBillingForbidden:true,
        electronicInvoicingCompatibilityRequired:true
      },

      tax:{
        vatTreatment:"server_validate_actual_status",
        fiscalQualification:"server_validate_actual_operation",
        associationTaxReceipt:"only_if_legally_eligible"
      },

      audit:{
        agent1Required:true,
        agent2IndependentControlRequired:true,
        activationAfterControls:true,
        securityLogRequired:true
      }
    };
  }
  ...
  function sportFinanceOpenReview(){
    ...
    const clubSnapshot =
      sportFinanceClubSnapshot(club);

    const merchantSnapshot =
      sportFinanceMerchantSnapshot(profile);

    const associationSnapshot =
      sportFinanceAssociationSnapshot(association);
  ...
  function sportFinanceEditIdentity(context){
    activeCorrectionDraftId =
      sportFinanceText(
        context && context.draftId
      );

    const firstField =
      sportFinanceField("bcfSportMerchantName");

    const returnButton =
      sportFinanceField("bcfSportReturnToReview");

    const correctionNotice =
      sportFinanceField("bcfSportCorrectionNotice");
  ...
  /* =========================================================
     ...
     IMPORTANT : le cœur Finance transmet volontairement
     un jeu minimal de données au connecteur.

     Ici, le raccord Sport recharge le brouillon gelé
     afin de réassocier au paiement :
     - commerçant ;
     - club ;
     - représentant ;
     ...
     Le serveur doit tout revalider.
     ========================================================= */
  ...
  function sportFinanceCheckoutPayload(request){
    ...
    return Object.assign(
      {},
      sportFinanceClone(base) || {},
      {
        connectorName:
          CONNECTOR_NAME,

        operationRef:
          sportFinanceText(draft.operationRef),

        representativeRef:
          sportFinanceText(draft.representativeRef),

        representative:
          sportFinanceClone(draft.representative) || null,

        presentedAt:
          sportFinanceText(draft.presentedAt),
        ...
        clubRef:
          sportFinanceText(draft.clubRef),

        clubSnapshot:
          sportFinanceClone(draft.clubSnapshot) || null,

        associationId:
          sportFinanceText(draft.associationId),

        associationSnapshot:
          sportFinanceClone(draft.associationSnapshot) || null,

        publicationDays:
          Number(draft.publicationDays || PUBLICATION_DAYS),

        financePolicies:
          sportFinanceClone(draft.financePolicies) ||
          sportFinancePolicies(),
  ...
  async function sportFinanceStartCheckout(request){
    ...
    const payload =
      sportFinanceCheckoutPayload(request);

    let responseData = null;

    if(
      sportFinanceText(config.mode)
      ...
      const response = await fetch(
        sportFinanceText(config.apiBaseUrl)
          .replace(/\/$/, "") +
          "/payments/checkout",
        {
          method:"POST",
    ...
    if(
      ...
      !sportFinanceText(responseData.paymentReference)
    ){
      throw new Error(
    ...
    sportFinanceUpsertOperation({
      draftId:
        sportFinanceText(payload.draftId),

      operationRef:
        sportFinanceText(payload.operationRef),
      ...
      amountHT:
        Number(payload.amountHT || 0),

      allocationCode:
        sportFinanceText(payload.allocationCode),

      clubRef:
        sportFinanceText(payload.clubRef),

      representativeRef:
        sportFinanceText(payload.representativeRef),

      associationId:
        sportFinanceText(payload.associationId),

      publicationDays:
        Number(payload.publicationDays || PUBLICATION_DAYS)
    });
  ...
  function sportFinanceRender(){
      ...
      <div class="sportCard">
        ...
        <div class="sportTitleText">
          ...
          <span class="bociteSportLogo">
            Bo'Cité<span class="bociteSportArt">Art</span>
          </span>
        </div>

        <div class="sportText" style="margin-top:10px;">
          Le commerçant choisit un montant à partir de
          <strong>50 € HT</strong>.
          La diffusion démarre uniquement après confirmation effective
          du paiement et dure <strong>5 jours réels</strong>.
        </div>

        <div class="sportStatus" style="margin-top:12px;">
          ...
          ${
            representative.team
              ? " — Équipe : " +
                sportFinanceEscape(representative.team)
              : ""
          }
        </div>
        ...
        <div class="sportCard">
          ...
          <div class="sportStatus">
            Relisez attentivement ces informations.
            Elles servent au paiement, à la publicité et aux documents associés.
            La qualification fiscale définitive reste contrôlée selon la situation réelle
            de chaque partie.
          </div>
          ...
          <input
            ...
            class="sportField"
            value="${sportFinanceEscape(profile.address)}"
          >
          ...
          <input
            ...
            class="sportField"
            type="tel"
            value="${sportFinanceEscape(profile.phone)}"
          >
          ...
          <input
            ...
            class="sportField"
            type="email"
            value="${sportFinanceEscape(profile.email)}"
          >
        ...
        <div class="sportCard">
          ...
          <div class="sportSubTitle">
            Montant et destination du parrainage
          </div>
          ...
          <label class="sportCheck">
            ...
            >
            <span>
              <strong>100 % pour le club</strong>
            </span>
          </label>
          ...
          <label class="sportCheck">
            <input
              ...
              name="bcfSportAllocation"
              value="HALF_HALF"
              ${associations.length ? "" : "disabled"}
            >
            <span>
              <strong>
                50 % pour le club / 50 % pour l’association choisie
              </strong>
            </span>
          </label>
          ...
          <div class="sportStatus" style="margin-top:12px;">
            Aucun règlement, reversement, facture ou reçu fiscal
            n’est considéré comme définitif dans le navigateur.
            Les documents et traitements correspondants sont établis
            après contrôle du paiement et du statut réel des parties.
          </div>
    ...
    const openReview =
      sportFinanceField("bcfSportOpenReview");
  ...
  function sportFinanceHandleStatus(
    ...
    sportFinanceUpsertOperation({
      draftId:
        sportFinanceText(draft.draftId),

      operationRef:
        sportFinanceText(draft.operationRef),

      paymentReference:
        sportFinanceText(draft.paymentReference),

      serverReference:
        sportFinanceText(draft.serverReference),

      status:
        status,

      amountHT:
        Number(draft.amountHT || 0),

      allocationCode:
        sportFinanceText(draft.allocationCode),
    ...
    sportFinanceSetStatus(
      messages[status] ||
      sportFinanceStatusLabel(status),
      (
        status === "refused" ||
        status === "cancelled" ||
        status === "disputed"
